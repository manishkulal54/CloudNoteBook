...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "error", error: "error occured while logging in" });
  }
});
...
router.post("/getdata", LoginStatus, async (req, res) => {
  try {
    ...
    res.status(501).json({ message: "success", user });
  } catch (e) {
    res
      .status(500)
      .json({
        message: "error",
        error: "error occured while getting user data",
      });
  }
});
...
router.post("/fetchalluser", async (req, res) => {
  try {
    ...
    if (!users) return res.status(100).json({ message: "failed" });
    ...
    res
      ...
      .json({
        message: "error",
        error: "error occured while fetching all the user",
      });
  }
});
